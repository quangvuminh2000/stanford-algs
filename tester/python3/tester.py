    ...
    *alg_path, alg_file = student_algorithm.split("/")
    ...
    for file_size, fileref, expected_output in test_cases:
        ...
        if len(alg_output) != len(expected_output):
            print(
                "Error. Your algorithm should be returning",
                len(expected_output),
                "values, but it is returning",
                len(alg_output),
                "instead. Fix this " + "and run the tester again.",
            )
            return
    ...
    if tests_total == tests_passed:
        ...
        print("\N{heavy check mark}" * len(message))
        print(message)
        print("\N{heavy check mark}" * len(message))
    else:
        message = "Some tests didn't pass"
        print("\N{heavy ballot x}" * len(message))
        print(message)
        print("\N{heavy ballot x}" * len(message))
